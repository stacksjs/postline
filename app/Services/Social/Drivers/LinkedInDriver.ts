 ...
 *
 * Everything else — OAuth `getAuthUrl`/`getProfile`, post deletion, history
 * enumeration, the versioned REST plumbing — is inherited unchanged. Only
 * `publish` is overridden, plus a `uploadImage` step and the refresh token the
 * base driver drops.
 */
