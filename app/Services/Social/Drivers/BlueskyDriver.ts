  ...
  async uploadBlob(identity: SocialIdentityCredentials, bytes: Uint8Array, mimeType: string): Promise<unknown> {
    ...
    const payload = await this.request<{ blob: unknown }>(
      new URL(`${this.service}/xrpc/com.atproto.repo.uploadBlob`),
      {
        ...
        // Uint8Array is a valid fetch body at runtime; the DOM lib types
        // only list URLSearchParams-ish BodyInit, hence the cast.
...
// Consumers historically imported this as `BlueskyDriver` (the vendored
// wrapper aliased it); keep that name stable.
export { BlueskyPublishingDriver as BlueskyDriver }
