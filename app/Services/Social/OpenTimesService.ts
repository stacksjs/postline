/**
 * The Open Times as a publishing target.
 *
...
export class OpenTimesService {
  async status(): Promise<{ provider: 'opentimes', handle: string, canPublish: boolean, characterLimit: number, listed: boolean }> {
  ...
  async purgeAdapter(): Promise<ProviderPurgeAdapter> {
    ...
    return {
      provider: 'opentimes',
      // The Open Times has no `social_identities` row: it is not an external account
      ...
      deletePost: async (ref: RemotePostRef): Promise<void> => {
        const id = Number(String(ref.uri || '').replace(/^opentimes:/, ''))
        if (!id) throw new Error('That is not an Open Times entry.')
  ...
  async publishToPost(
    ...
    try {
      if (post.body.length > CHARACTER_LIMIT)
        throw new Error(`The Open Times short posts must be ${CHARACTER_LIMIT} characters or fewer. Publish it to your blog instead.`)
      ...
      await database.updateTable('post_targets').set({
        status: 'published',
        remote_uri: entry ? `opentimes:${entry.id}` : `opentimes:unlisted:${post.id}`,
        failure_reason: null,
        updated_at: now(),
      ...
      return {
        ...
        url: url || undefined,
        uri: `opentimes:${entry?.id ?? publication.id}`,
      ...
      return { provider: 'opentimes', ok: false, error: message, targetId: Number(target.id) }
    }
  }
}

export const opentimes = new OpenTimesService()
