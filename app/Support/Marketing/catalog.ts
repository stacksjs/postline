...
export const FEATURES: MarketingEntry[] = [
  ...
  },
  {
    slug: 'self-hosting',
    nav: 'Self-hosting',
    title: 'Your keys, your server',
    summary: 'Run Postline on infrastructure you control. No middleman holds your tokens or sits between you and your audience.',
    icon: 'server-stack-01',
    intro: 'Every crossposting service asks you to hand over write access to your social accounts. Postline is the same software, running on your own box, with the tokens in your own database.',
    sections: [
      { heading: 'Tokens never leave your instance', body: 'OAuth tokens and app passwords are stored encrypted in your database, with a key only your instance holds. There is no vendor with a copy.' },
      { heading: 'One command to deploy', body: 'Postline deploys as a single Bun service. There is no queue cluster, no broker, and no separate worker fleet to keep alive.' },
    ],
    points: [
      'Tokens encrypted with your own key',
      'Single Bun service, no broker to babysit',
      'MIT licensed, fork it if you want',
    ],
  },
]
...
export const USE_CASES: MarketingEntry[] = [
  {
    ...
    sections: [
      ...
      { heading: 'Keep the record', body: 'Every published post and its engagement stays in your own database, so a year of building in public is something you own rather than something you rent.' },
    ],
    points: [
  ...
  },
  {
    ...
    summary: 'Announce releases across networks from a project you can read, fork and run yourself.',
    icon: 'source-code',
    intro: 'Maintainers already run their own infrastructure and already distrust closed tooling. Postline is MIT licensed and self-hosted, which makes it something you can actually audit before giving it publishing rights.',
    sections: [
  ...
  },
  {
    ...
    summary: 'Manage posting for a handful of accounts on your own instance, without a per-seat subscription.',
    icon: 'building-03',
    intro: 'Scheduling tools price by seat and by account, and hold the access tokens for clients you are responsible for. Running your own instance removes both problems at once.',
    sections: [
      { heading: 'No per-seat pricing', body: 'The cost of an instance does not change with the number of accounts connected to it, because there is no vendor in the middle metering it.' },
      { heading: 'Clean handover', body: 'When an engagement ends, the tokens are revoked on your instance and the posting history goes with the client rather than staying locked in a platform.' },
    ],
    points: [
      'Cost does not scale per seat',
      'Client tokens stay on infrastructure you control',
